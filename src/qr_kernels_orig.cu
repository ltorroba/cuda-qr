...
#define numilpqr 2
#define numilpqr2 1
#define numthreadsqr 16
#define numthreadsqr2 32



namespace qr_improved {
...
//-------------------- multiply by Q kernels-----------------------------------
    ...
    __global__ void __launch_bounds__(tilesize) applyQt_singletile( //aplies Qt (given by householder reflectors on diagonal tile k) to the remainder of the row
        int size_in,
        int size_out,
        int diag_iter,
        bool offsetdiag,
    ...
    __global__ void __launch_bounds__(tilesize) applyQt_doubletile( //aplies Qt (given by householder reflectors on the tile at row_idx below diag_idx) to the remainder of the row, and to the row of diag_idx
        int size_in,
        int size_out,
        int diag_iter,
        int row_iter,
...
__global__ __launch_bounds__(numthreadsqr) void calcQR_singletile( //calculates in-place QR of diagonal tile
    int size_in,
    int diag_iter,
    float *tau,
    float *out) {
...
__global__ __launch_bounds__(numthreadsqr2) void calcQR_doubletile( //calculates in-place QR of diagonal tile combined with row_idx tile below
    int size_in,
    int diag_iter,
    int row_iter,
    float *tau,
    ...
    float outs[2*tilesize][numilpqr2];
    float taureg[numilpqr2];
    __shared__ float cache[tilesize+2];
    int diagstartidx=diag_iter*tilesize;
    int iteroffset=row_iter*tilesize;

    for (int j=i; j<tilesize; j+=numthreadsqr2){
        for (int k=0; k<=tilesize;k++){
            outs[k][j/numthreadsqr2]=out[(k+diagstartidx)*size_in+j+diagstartidx];
        }
        for (int k=0; k<tilesize;k++){
            outs[k+tilesize][j/numthreadsqr2]=out[(k+diagstartidx+iteroffset)*size_in+j+diagstartidx];
        }
    }
    
    
    for(int iter=0;iter<tilesize;iter++){
        float tmp_sum[numilpqr2]={0.0f};
        if (i==(iter%numthreadsqr2)){
            for (int k=0;k<tilesize;k++){
                cache[k]=outs[k+tilesize][iter/numthreadsqr2];
                tmp_sum[iter/numthreadsqr2]+=outs[k+tilesize][iter/numthreadsqr2]*outs[k+tilesize][iter/numthreadsqr2];
            }
            cache[tilesize]=tmp_sum[iter/numthreadsqr2];
            cache[tilesize+1]=outs[iter][iter/numthreadsqr2];
        }
        __syncthreads();
        for (int j=i; j<tilesize; j+=numthreadsqr2){
            if (j>=iter){
                ...
                if (j>iter){
                    for (int k=0;k<tilesize;k++){
                        tmp_sum[j/numthreadsqr2]+=cache[k]*outs[k+tilesize][j/numthreadsqr2];
                    }
                }else{
                    taureg[iter/numthreadsqr2]=taut;
                }

                float tmp_sum2 = (tmp_sum[j/numthreadsqr2] / newvalue+outs[iter][j/numthreadsqr2])*taut;
                outs[iter][iter/numthreadsqr2]-=tmp_sum2;
                if (j>iter){
                    for (int k=0;k<tilesize;k++){
                        outs[k+tilesize][j/numthreadsqr2]*=newvalue;
                        outs[k+tilesize][j/numthreadsqr2]-=cache[k]* tmp_sum2;
                    }
                }
                for (int k=0;k<tilesize;k++){
                    outs[k+tilesize][j/numthreadsqr2]/=newvalue;
    ...
    for (int j=i; j<tilesize; j+=numthreadsqr2){
        for (int k=0; k<=tilesize;k++){
            out[(k+diagstartidx)*size_in+j+diagstartidx]=outs[k][j/numthreadsqr2];
        }
        for (int k=0; k<tilesize;k++){
            out[(k+diagstartidx+iteroffset)*size_in+j+diagstartidx]=outs[k+tilesize][j/numthreadsqr2];
        }
        tau[(diag_iter)*size_in+row_iter*tilesize+j]=taureg[j/numthreadsqr2];
    }
...
/*
    ...
    void launch_mult_qt(
        ...
        for(int iter=0;iter<min(size_i,size_j)/tilesize;iter++){
            applyQt_singletile<<<no_blocks,dim3(tilesize)>>>(size_j,size_k,iter,false,tau,a, b); 
            for (int row=1;row+iter<(size_i/tilesize);row++){

                applyQt_doubletile<<<no_blocks,dim3(tilesize)>>>(size_j,size_k,iter,row,false,tau,a, b); 
            }
        }
    ...
    void test_qrkernel_single(
        int32_t size_i,
        float *a, float *tau) {
        calcQR_singletile<<<1,dim3(numthreadsqr)>>>(size_i,0,tau,a); 
    ...
    void test_mulqtkernel_single(
        ...
        float *a, float *tau) {
            calcQR_singletile<<<1,dim3(numthreadsqr)>>>(size_i,0,tau,a); 
            applyQt_singletile<<<1,dim3(tilesize)>>>(size_i,size_i,0,true,tau,a,a); 
    ...
    void test_qrkernel_double(
        ...
        float *a, float *tau) {
        calcQR_singletile<<<1,dim3(numthreadsqr)>>>(size_i,0,tau,a); 
        calcQR_doubletile<<<1,dim3(numthreadsqr2)>>>(size_i,0,1,tau,a); 
    ...
    void test_mulqtkernel_double(
        ...
        float *a, float *tau) {
            ...
            applyQt_singletile<<<1,dim3(tilesize)>>>(size_i,size_i,0,true,tau,a,a); 
            calcQR_doubletile<<<1,dim3(numthreadsqr2)>>>(size_i,0,1,tau,a);
            applyQt_doubletile<<<1,dim3(tilesize)>>>(size_i,size_i,0,1,true,tau,a,a); 
            calcQR_singletile<<<1,dim3(numthreadsqr)>>>(size_i,1,tau,a); 
...
struct QRimproved {
    constexpr static char const *name = "qr_improved";
    ...
    run(int32_t size_i,
        float *a, float *tau) {
        qr_improved::launch_tiled_qr(size_i,a, tau) ;
    }

    static void
    run_mulqt(int32_t size_k,int32_t size_i, int32_t size_j, 
        float *a, float *tau, float *b) {
        qr_improved::launch_mult_qt(size_k,size_i,size_j,a, tau, b) ;
    }
    ...
    testqr(int32_t size_i,
        float *a, float *tau) {
        qr_improved::test_qrkernel_single(size_i,a, tau) ;
    }

    static void
    testqr2(int32_t size_i,
        float *a, float *tau) {
        qr_improved::test_qrkernel_double(size_i,a, tau) ;
    }
    static void
    testmulq(int32_t size_i,
        float *a, float *tau) {
        qr_improved::test_mulqtkernel_single(size_i,a, tau) ;
    }

    static void
    testmulq2(int32_t size_i,
        float *a, float *tau) {
        qr_improved::test_mulqtkernel_double(size_i,a, tau) ;
    }
};
